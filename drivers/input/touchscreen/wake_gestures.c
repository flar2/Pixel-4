...
#define SWEEP_X_B1              350
#define SWEEP_X_B2              600
#define SWEEP_X_START		530
#define SWEEP_X_FINAL           260
...
//get hardware type
static int hw_version = REDFIN;
static int __init get_model(char *cmdline_model)
{
	if (strstr(cmdline_model, "bramble")) {
		hw_version = BRAMBLE;
	}
	return 0;
}
...
#if WG_DEBUG
        pr_info(LOGTAG"x,y(%4d,%4d) tap_time_pre:%llu ktime:%llu\n",
                x, y, tap_time_pre, ktime_to_ms(ktime_get()));
#endif
